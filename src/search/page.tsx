  ...
  useEffect(() => {
    ...
    if (query) {
      ...
      for (const act of legalActs) {
        const sections = act.chapters ? act.chapters.flatMap(c => c.sections.map(s => ({section: s, chapter: c}))) : (act.sections || []).map(s => ({section: s}));
        for (const {section, chapter} of sections) {
          const chapterTitle = chapter?.chapter_title;
          const chapterNumber = chapter?.chapter_number;
          if (
            act.act_name.toLowerCase().includes(lowerCaseQuery) ||
            (chapterTitle && chapterTitle.toLowerCase().includes(lowerCaseQuery)) ||
                    ...
                    <Link key={`${act.act_name}-${section.section_number}`} href={`/act/${slugify(act.act_name)}/section/${section.section_number}`}>
                      <Card className="hover:bg-secondary transition-colors hover:border-primary">
                        <CardHeader>
                          ...
                          <CardDescription>
                            {act.act_name} 
                            {chapter && ` → Chapter ${chapter.chapter_number}${chapter.chapter_title ? `: ${chapter.chapter_title}` : ''}`}
                          </CardDescription>
                        </CardHeader>
                      </Card>
                    </Link>
